package com.takisjoeapp.laundryaja.feature.customer.present.ui.update;

import androidx.lifecycle.ViewModelProvider;

import android.os.Bundle;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.takisjoeapp.laundryaja.R;

public class UpdateCustomerFragment extends Fragment {

    private UpdateCustomerViewModel mViewModel;

    public static UpdateCustomerFragment newInstance() {
        return new UpdateCustomerFragment();
    }

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_update_customer, container, false);
    }

    @Override
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        super.onActivityCreated(savedInstanceState);
        mViewModel = new ViewModelProvider(this).get(UpdateCustomerViewModel.class);
        // TODO: Use the ViewModel
    }

}